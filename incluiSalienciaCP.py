...
workfolder = Path('')
saliency = Path("CPSaliency")
video_path = Path("VQA-ODV")
saida_path = Path("VQA-ODV-CP-Preprocess")
...
for video in video_path.glob('*.mp4'):
  ...
  salimage = saliency / raiz / "saliency_maps" 
  ...
  for frame in range(1,600):
    saliency_name_list.append(f'{frame}.jpg')
  ...
  for frame, (frame_array, saliency_name) in enumerate(zip(vreader, saliency_name_list)):
    ...
    img_sal = Image.open(saliency/raiz/saliency_name).convert('L')
